export const lineraInstructions = () => {
    ...
    # 只要动了代码就必须运行单元测试
    只要修改了代码，无论是增加函数，删除函数，还是变更函数实现，都必须运行单元测试，确保所有单元测试通过。未通过单元测试的代码将被拒绝。
    ...
    # 文档管理
    开发过程中创建的所有设计文档，TODO列表，需求分析文档等，必须用Markdown格式编写，管理在和项目顶层Cargo.toml处于同级的documentation目录下。迭代过程中需要根据进度更新文档内容。
    `,
	].join("\n\n")
}
